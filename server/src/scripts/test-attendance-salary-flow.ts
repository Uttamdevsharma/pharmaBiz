...
async function runTest() {
  ...
  await AccountingService.setSalaryConfig(tenant.id, {
    branchId: branch.id,
    userId: staff.id,
  ...
  await (prisma as any).employeeMonthlyAllowance.deleteMany({
    where: { userId: staff.id, month: testMonth },
  });
  await (prisma as any).salaryDisbursement.deleteMany({
    where: { userId: staff.id, month: testMonth },
  });
  await (prisma as any).employeeAttendance.deleteMany({
    where: { userId: staff.id, date: { startsWith: testMonth } },
  });

  // 3. Configure Monthly Off-Days: Friday (5) & Saturday (6)
  await AttendanceService.setBranchOffDayConfig(
    tenant.id,
    branch.id,
    testMonth,
    {
  ...
  await AttendanceService.markBulkDailyAttendance(
    tenant.id,
    branch.id,
    "2026-09-01",
    {
  ...
  const calc = await AttendanceService.calculateMonthlySalary(tenant.id, branch.id, staff.id, testMonth);
  ...
  const disburseResult = await AccountingService.disburseSalary(
    tenant.id,
    manager.id,
    {
      branchId: branch.id,
  ...
  const savedDisbursement = await prisma.salaryDisbursement.findUnique({
  ...
  const branchHistory = await AccountingService.getBranchSalaryHistory(tenant.id, branch.id, {
    month: testMonth,
    userId: staff.id,
  });
  ...
  const employeeHistory = await AccountingService.getEmployeeSalaryHistory(tenant.id, staff.id);
