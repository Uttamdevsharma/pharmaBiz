...
export const MULTI_BRANCH_ROLES = [
  "COMPANY_OWNER",
  "SUPER_ADMIN",
  "REGIONAL_ADMIN",
  "CTO",
...
export async function resolveBranchContext(
  req: Request
): Promise<{ branchId?: string; isAllBranches: boolean }> {
  if (!req.user) {
    return { branchId: undefined, isAllBranches: true };
  }

  const user = req.user;
  const isMultiBranchAllowed = MULTI_BRANCH_ROLES.includes(user.role);

  // Extract requested branch from headers, query, or body
  const headerBranch = (req.headers["x-branch-id"] as string)?.trim();
  const queryBranch = (req.query.branchId as string)?.trim();
  ...
  if (!isMultiBranchAllowed && user.branchId) {
    ...
    if (requestedBranch && requestedBranch !== "all" && requestedBranch !== user.branchId) {
      const error: any = new Error("Access denied: You are only authorized to access your assigned branch.");
      error.status = 403;
      throw error;
    }
  ...
  if (isMultiBranchAllowed) {
    ...
    if (!branch) {
      const error: any = new Error("Requested branch does not exist or is inactive for your organization.");
      error.status = 400;
      throw error;
    }
 ...
 */
export const enforceBranchContext = (options: { required?: boolean } = {}) => {
