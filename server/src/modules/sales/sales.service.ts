...
export class SalesService {
   ...
   */
  static async createSale(tenantId: string, userId: string, data: CreateSaleInput) {
    ...
    const now = new Date();
    const availableBatches = await (prisma as any).inventory.findMany({
      where: {
   ...
   */
  static async getCustomers(tenantId: string, search?: string) {
    const where: any = {
      tenantId,
    ...
    const customerMap = new Map<string, { phone: string; name: string; email?: string; address?: string }>();
   ...
   */
  static async getReceiptData(saleId: string, tenantId: string) {
    const [sale, tenant] = await Promise.all([
      (prisma as any).sale.findFirst({
        where: { id: saleId, tenantId },
        include: {
          branch: true,
          user: { select: { id: true, name: true, username: true } },
  ...
  static async refundSale(
    ...
    const refunded = await (prisma as any).$transaction(async (tx: any) => {
  ...
  static async voidSale(
    ...
    const voided = await (prisma as any).$transaction(async (tx: any) => {
      const updatedSale = await tx.sale.update({
        where: { id: saleId },
        data: {
          status: "VOIDED",
   ...
   */
  static async collectDue(
    tenantId: string,
    userId: string,
    saleId: string,
    data: CollectDueInput
  ) {
    const sale = await (prisma as any).sale.findFirst({
      where: { id: saleId, tenantId },
      include: { branch: true },
    ...
    const updatedSale = await (prisma as any).$transaction(async (tx: any) => {
   ...
   */
  static async getDueStats(tenantId: string, branchId?: string | null) {
    const where: any = { tenantId, status: "COMPLETED", dueAmount: { gt: 0 } };
    if (branchId && branchId !== "all" && branchId !== "all-branches") {
      where.branchId = branchId;
    }

    const aggregate = await (prisma as any).sale.aggregate({
      where,
      _sum: {
        dueAmount: true,
        totalAmount: true,
