...
import {
  SetBranchOffDayConfigInput,
  MarkBulkDailyAttendanceInput,
...
export class AttendanceService {
   ...
   */
  static getMonthDaysAndOffDays(month: string, weeklyOffDays: string[] = ["FRIDAY"], customOffDates: string[] = []) {
   ...
   */
  static async getBranchOffDayConfig(tenantId: string, branchId: string, month: string) {
    const config = await (prisma as any).branchOffDayConfig.findUnique({
      where: {
   ...
   */
  static async setBranchOffDayConfig(
    tenantId: string,
    branchId: string,
    month: string,
    data: SetBranchOffDayConfigInput,
    actorId: string
  ) {
    const weeklyOffDays = data.weeklyOffDays.map((d) => d.trim().toUpperCase());
    const customOffDates = (data.customOffDates || []).map((d) => d.trim());

    const saved = await (prisma as any).branchOffDayConfig.upsert({
      where: {
        branchId_month: { branchId, month },
      },
      update: {
   ...
   */
  static async getDailyAttendanceSheet(tenantId: string, branchId: string, date: string) {
    ...
    const existing = await (prisma as any).employeeAttendance.findMany({
      where: {
        tenantId,
        branchId,
        date,
    ...
    const recordMap = new Map(existing.map((r: any) => [r.userId, r]));
   ...
   */
  static async markBulkDailyAttendance(
    tenantId: string,
    branchId: string,
    date: string,
  ...
  static async getEmployeeAttendanceHistory(tenantId: string, userId: string, month: string, requestingUser?: any) {
    ...
    const attendances = await (prisma as any).employeeAttendance.findMany({
      where: {
        tenantId,
        userId,
        date: {
    ...
    const recordMap = new Map(attendances.map((a: any) => [a.date, a]));
    ...
    const history = calendarDays.map((day) => {
      const record: any = recordMap.get(day.date);
      let status: string;
   ...
   */
  static async calculateMonthlySalary(tenantId: string, branchId: string, userId: string, month: string) {
    const employee = await (prisma as any).user.findFirst({
      where: { id: userId, tenantId },
      include: {
        salaryConfig: true,
    ...
    const monthlyAllowances = await (prisma as any).employeeMonthlyAllowance.findMany({
      where: {
        tenantId,
        branchId,
        userId,
   ...
   */
  static async listEmployeeAllowances(tenantId: string, branchId: string, userId: string, month: string) {
    return (prisma as any).employeeMonthlyAllowance.findMany({
      where: { tenantId, branchId, userId, month },
      orderBy: { createdAt: "desc" },
    });
  }

  static async addEmployeeAllowance(
    tenantId: string,
    branchId: string,
    userId: string,
    month: string,
    ...
    const allowance = await (prisma as any).employeeMonthlyAllowance.create({
      data: {
        tenantId,
        branchId,
        userId,
  ...
  static async deleteEmployeeAllowance(tenantId: string, allowanceId: string, actorId: string) {
    const existing = await (prisma as any).employeeMonthlyAllowance.findFirst({
      where: { id: allowanceId, tenantId },
    });
    if (!existing) throw new Error("Allowance not found.");

    await (prisma as any).employeeMonthlyAllowance.delete({
  ...
  static async reactivateEmployee(tenantId: string, branchId: string, userId: string, actorId: string) {
    const employee = await (prisma as any).user.findFirst({
      where: { id: userId, tenantId, branchId },
    });
    if (!employee) throw new Error("Employee not found in this branch.");
