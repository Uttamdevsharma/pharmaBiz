...
class TransferService {
    ...
    static async createTransfer(tenantId, userId, data) {
        ...
        for (const item of data.items) {
            let inv = null;
            if (item.inventoryId) {
                inv = await prisma_1.prisma.inventory.findFirst({
                    where: {
                        id: item.inventoryId,
    ...
    static async receiveTransfer(transferId, tenantId, userId, data) {
        ...
        for (const item of transfer.items) {
            ...
            if (totalAccounted !== item.sentQuantity) {
                throw new Error(`Quantities for product item mismatch sent amount. Sent: ${item.sentQuantity}, Received + Damaged + Missing: ${totalAccounted}.`);
            }
            ...
            if (receivedQty === 0 && damagedQty > 0)
                itemStatus = "DAMAGED";
            else if (receivedQty === 0 && missingQty > 0)
                itemStatus = "MISSING";
            else if (damagedQty > 0 || missingQty > 0)
        ...
        const finalized = await prisma_1.prisma.$transaction(async (tx) => {
    ...
    static async settleTransfer(transferId, tenantId, userId, data) {
        ...
        if (settlementAmount > currentRemainingDue + 0.05) {
            throw new Error(`Settlement amount (৳${settlementAmount.toFixed(2)}) exceeds remaining due (৳${currentRemainingDue.toFixed(2)}).`);
        }
        // Validate paying account belongs to Destination Branch
        const sourceAccount = await prisma_1.prisma.financialAccount.findFirst({
            where: {
        ...
        const destAccount = await prisma_1.prisma.financialAccount.findFirst({
            where: {
