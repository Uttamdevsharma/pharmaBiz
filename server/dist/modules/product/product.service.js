...
exports.ProductService = exports.CANONICAL_MAIN_CATEGORIES = void 0;
exports.mapCategoryNameToProductType = mapCategoryNameToProductType;
const prisma_1 = require("../../app/lib/prisma");
const audit_1 = require("../../app/lib/audit");
exports.CANONICAL_MAIN_CATEGORIES = [
    {
        name: "Medicine",
...
function mapCategoryNameToProductType(categoryName) {
    ...
    if (lower.includes("syrup") || lower.includes("liquid") || lower.includes("suspension") || lower.includes("drop"))
        return "SYRUP";
    if (lower.includes("equipment") || lower.includes("device") || lower.includes("monitor") || lower.includes("surgical") || lower.includes("disposable"))
        return "EQUIPMENT";
    if (lower.includes("saline") || lower.includes("fluid") || lower.includes("infusion") || lower.includes("iv"))
        return "SALINE";
    if (lower.includes("other") || lower.includes("care") || lower.includes("hygiene") || lower.includes("baby") || lower.includes("supplement") || lower.includes("general"))
        return "OTHER";
    return "MEDICINE";
}
class ProductService {
        ...
        try {
            ...
            for (const prod of products) {
                ...
                if (prod.categoryRef && prod.categoryRef.parentId) {
                    ...
                    const parent = await prisma_1.prisma.category.findUnique({
    ...
    static async createCategory(tenantId, userId, data) {
        ...
        if (data.parentId) {
            parentCategory = await prisma_1.prisma.category.findFirst({
    ...
    static async listUnits(tenantId) {
        let units = await prisma_1.prisma.unit.findMany({
            where: { tenantId },
            orderBy: { name: "asc" },
        });
        if (units.length === 0) {
            await this.seedDefaultCatalogVariants(tenantId);
            units = await prisma_1.prisma.unit.findMany({
                where: { tenantId },
                orderBy: { name: "asc" },
            });
        }
    ...
    static async createProduct(tenantId, userId, data) {
        ...
        if (mainCategoryId) {
            const cat = await prisma_1.prisma.category.findUnique({ where: { id: mainCategoryId } });
            if (cat) {
                // If user passed a subcategory as categoryId by mistake, resolve parent
                if (cat.parentId) {
                    subcategoryId = cat.id;
                    subcategoryName = cat.name;
                    const parent = await prisma_1.prisma.category.findUnique({ where: { id: cat.parentId } });
                    if (parent) {
        ...
        if (subcategoryId && !subcategoryName) {
            const sub = await prisma_1.prisma.category.findUnique({ where: { id: subcategoryId } });
            if (sub) {
                subcategoryName = sub.name;
                if (!mainCategoryId && sub.parentId) {
                    mainCategoryId = sub.parentId;
                    const parent = await prisma_1.prisma.category.findUnique({ where: { id: sub.parentId } });
                    if (parent)
                        mainCategoryName = parent.name;
                }
            }
        ...
        if (!mainCategoryName && !mainCategoryId) {
            ...
            const root = await prisma_1.prisma.category.findFirst({
                where: { tenantId, name: "Medicine", parentId: null },
            });
            if (root)
                mainCategoryId = root.id;
        }
        let brandName = data.brandName || null;
        if (data.brandId && !brandName) {
            const b = await prisma_1.prisma.brand.findUnique({ where: { id: data.brandId } });
            if (b)
                brandName = b.name;
        }
    ...
    static async updateProduct(productId, tenantId, userId, data) {
        ...
        if (data.subcategoryId !== undefined && data.subcategoryId !== product.subcategoryId) {
            if (data.subcategoryId === null || data.subcategoryId === "") {
                ...
                const sub = await prisma_1.prisma.category.findUnique({ where: { id: data.subcategoryId } });
                if (sub) {
    ...
    static async setBranchPriceOverride(productId, tenantId, userId, data) {
        ...
        const override = await prisma_1.prisma.branchProduct.upsert({
            where: {
                branchId_productId: {
    ...
    static async removeBranchPriceOverride(productId, branchId, tenantId, userId) {
        const override = await prisma_1.prisma.branchProduct.findUnique({
            where: {
                branchId_productId: {
                    branchId,
                    productId,
        ...
        await prisma_1.prisma.branchProduct.delete({
            where: {
                branchId_productId: {
                    branchId,
                    productId,
