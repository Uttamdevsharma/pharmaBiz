...
class ReportService {
    ...
    static async getDailySales(tenantId, query, userRole, userBranchId) {
        ...
        const totalTaxes = sales.reduce((sum, s) => sum + Number(s.tax), 0);
        const totalTransactions = sales.length;
        const paymentMethods = {
            CASH: sales.filter((s) => s.paymentMethod === "CASH").reduce((sum, s) => sum + Number(s.totalAmount), 0),
            CARD: sales.filter((s) => s.paymentMethod === "CARD").reduce((sum, s) => sum + Number(s.totalAmount), 0),
            MOBILE: sales.filter((s) => s.paymentMethod === "MOBILE").reduce((sum, s) => sum + Number(s.totalAmount), 0),
        };
        // Hourly distribution
        const hourlyMap = {};
        for (let i = 0; i < 24; i++)
